<?php
/**
 * Podcats Player shortcode generator functionality.
 *
...
class ShortCodeGen {
	...
	public function __construct() {
        ...
        $shortcode_settings = get_option( 'pp_shortcode_options' );
			...
			array(
				...
				'filter'    => array(
					...
					'items'       => array(
						...
						'autoplay' => array(
							'setting' => 'autoplay',
							'label'   => esc_html__( 'Autoplay Behavior', 'podcast-player' ),
							'type'    => 'select',
							'choices' => array(
								''        => esc_html__( 'Default (Follow visual list order)', 'podcast-player' ),
								'reverse'  => esc_html__( 'Reverse (Opposite of visual list order)', 'podcast-player' ),
								'disable' => esc_html__( 'Disabled (No autoplay)', 'podcast-player' ),
							),
						),
					),
				),
			...
			foreach ( $items as $item => $attr ) {
				...
				switch ( $type ) {
					case 'select':
						...
						break;
					case 'checkbox':
						$optmar  = sprintf( '<input class="pp-getval" name="%s" id="%s" type="checkbox" value="yes" %s />', $name, $id, checked( $instance[ $set ], 'yes', false ) );
						$optmar .= $this->label( $set, $label, false );
						break;
					case 'text':
						$optmar  = $this->label( $set, $label, false );
						$optmar .= sprintf( '<input class="widefat pp-getval" name="%1$s" id="%2$s" type="text" value="%3$s" />', $name, $id, esc_attr( $instance[ $set ] ) );
						$optmar .= sprintf( '<div class="pp-desc">%s</div>', $desc );
						break;
					case 'url':
						$optmar  = $this->label( $set, $label, false );
						$optmar .= sprintf( '<input class="widefat pp-getval" name="%1$s" id="%2$s" type="url" value="%3$s" %4$s />', $name, $id, esc_attr( $instance[ $set ] ), $inputattr );
						$optmar .= sprintf( '<div class="pp-desc">%s</div>', $desc );
						break;
					case 'number':
						$optmar  = $this->label( $set, $label, false );
						$optmar .= sprintf( '<input class="widefat pp-getval" name="%1$s" id="%2$s" type="number" value="%3$s" %4$s />', $name, $id, absint( $instance[ $set ] ), $inputattr );
						$optmar .= sprintf( '<div class="pp-desc">%s</div>', $desc );
						break;
					case 'mmss':
						$optmar  = $this->label( $set, $label, false );
						$optmar .= sprintf( '<div class="mmss-time" id="%s">', $id );
						$optmar .= sprintf( '<input class="tiny-text pp-getval" name="%1$s[]" type="number" value="%2$s" min="0" max="10" size="2"/> : ', $name, absint( $instance[ $set ][0] ) );
						$optmar .= sprintf( '<input class="tiny-text pp-getval" name="%1$s[]" type="number" value="%2$s"  min="0" max="59" size="2" /> : ', $name, absint( $instance[ $set ][1] ) );
						$optmar .= sprintf( '<input class="tiny-text pp-getval" name="%1$s[]" type="number" value="%2$s"  min="0" max="59" size="2" />', $name, absint( $instance[ $set ][2] ) );
						$optmar .= '</div>';
						$optmar .= sprintf( '<div class="pp-desc">%s</div>', $desc );
						break;
					case 'textarea':
						$optmar  = $this->label( $set, $label, false );
						$optmar .= sprintf( '<textarea class="widefat pp-getval" name="%1$s" id="%2$s" %3$s >%4$s</textarea>', $name, $id, $inputattr, esc_attr( $instance[ $set ] ) );
						break;
					case 'image_upload':
						$optmar  = $this->label( $set, $label, false );
					...
					case 'color':
						$optmar  = $this->label( $set, $label, false );
						$optmar .= sprintf( '<input class="pp-color-picker pp-getval" name="%1$s" id="%2$s" type="text" value="%3$s" />', $name, $id, esc_attr( sanitize_hex_color( $instance[ $set ] ) ) );
						break;
					case 'furl':
	...
	public function select( $sfor, $options, $selected, $classes = array(), $secho = true ) {
		...
		$select = sprintf(
			'<select id="%1$s" name="%2$s" class="podcast-player-%3$s widefat pp-getval">%4$s</select>',
	...
	public function image_upload( $id, $name, $value ) {
		...
		$markup .= sprintf( '<input class="podcast-player-widget-img-id pp-getval" name="%s" id="%s" value="%s" type="hidden" />', $name, $id, $value );
		return $markup;
	}
	...
	public function feed_url( $name, $id, $label, $args ) {
		...
		$markup .= sprintf( '<input class="pp_feed-url widefat pp-getval" name="%1$s" id="%2$s" type="text" value="%3$s" placeholder="%4$s" />', $name, $id, esc_attr( $selected ), esc_attr__( 'Enter Your Podcast Feed URL', 'podcast-player' ) );
		$markup .= '</div>';
		return $markup;
	}
	...
	public function mu_checkbox( $mufor, $options, $selected = array(), $classes = array(), $muecho = true ) {
		...
		foreach ( $rev_options as $id => $label ) {
			...
			$mu_checkbox .= "\n<li$final_class>" . '<label class="selectit"><input class="pp-getval" value="' . esc_attr( $id ) . '" type="checkbox" name="' . esc_attr( $this->get_field_name( $mufor ) ) . '[]"' . checked( in_array( strval( $id ), $selected, true ), true, false ) . ' /><span class="cblabel">' . esc_html( $label ) . "</span></label></li>\n";
		}
		$mu_checkbox .= "</ul></div>\n";
	...
	/**
	 * Handles updating the settings for the current shortcode instance.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 */
	public function sanitize( $new_instance, $old_instance = array() ) {
	...
	/**
	 * Handles escaping the settings for the current shortcode instance.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * @return array Updated settings to save.
	 */
	public function escape( $new_instance, $old_instance = array() ) {
		...
		foreach ( $sanitize_url as $setting ) {
			$instance[ $setting ] = esc_url( $new_instance[ $setting ] );
		}
		...
		if ( $this->is_premium && isset( $instance['pp_fetch_method'] ) && 'feed' !== $instance['pp_fetch_method'] ) {
			...
			if ( isset( $old_instance['feed_url'] ) && ( $new_instance['feed_url'] === $old_instance['feed_url'] ) ) {
				...
				$validurl = Validation_Fn::is_valid_url( $feedurl );
				if ( $validurl ) {
					$feedurl = esc_url( wp_strip_all_tags( $feedurl ) );
				} else {
					$feedurl = esc_html( $feedurl );
				}
			}

			$instance['feed_url'] = $feedurl;
			$instance['error']    = esc_html( $error );
		}

		return apply_filters( 'podcast_player_widget_escape', $instance, $new_instance, $this );
	}
	...
	public function dropdown() {
		...
		$select = array( '' => esc_html__( 'Select a Shortcode to Edit', 'podcast-player' ) );
		...
		foreach ( $select as $value => $label ) {
			$label = $label ? $label : esc_html__( 'Podcast Player Shortcode', 'podcast-player' ) . ' ' . ++$counter;
			$markup .= sprintf( '<option value="%1$s" %2$s>%3$s</option>', esc_attr( $value ), selected( $value, $selected, false ), esc_html( $label ) );
		}

		$markup = sprintf(
			'<select class="widefat pp-shortcode-dropdown">%1$s</select>',
			$markup
		);
	...
	public function save() {
		...
		update_option( 'pp_shortcode_options', $this->shortcode_settings );
	}
}
